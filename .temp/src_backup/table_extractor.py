...
extraction methods and includes automatic fallback for low-confidence tables. It also handles multi-page table detection and merging.
...
def extract_tables(
    ...
    try:
        ...
        if len(results) > 1 and IMPROVED_MERGER_AVAILABLE:
            logger.info(f"Applying multi-page table merging with strategy: {merge_strategy}")
            original_count = len(results)
            
            # Use our improved table merger with specified strategy
            results = process_and_merge_tables([_process_table(table) for table in tables], merge_strategy=merge_strategy)
