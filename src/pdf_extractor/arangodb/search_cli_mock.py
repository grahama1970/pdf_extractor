#!/usr/bin/env python3
# src/pdf_extractor/arangodb/search_cli_mock.py
...
# Import mock search functions
from pdf_extractor.arangodb.search_api.mock_search import mock_search_messages, mock_unified_search
...
def register_search_commands(app: typer.Typer) -> None:
    ...
    def unified_search_cmd(
        ...
        """
        logger.info(f"Executing unified search for: '{query}'")
        
        # Parse collections parameters
    ...
    def search_messages_cmd(
        ...
        """
        logger.info(f"Executing message search for: '{query}'")
        
        # Use mock search function
        results = mock_search_messages(
            query=query,
            search_type=search_type,
