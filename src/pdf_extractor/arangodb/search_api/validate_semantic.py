...
if __name__ == "__main__":
    ...
    if not query_embedding:
        logger.error("Validation failed: Could not create query embedding")
        sys.exit(1)
    ...
    # Define fixture path
    fixture_path = os.path.join(fixtures_dir, "semantic_test_fixture.json")
    ...
    # Check if collection has at least one document with embedding
    try:
        ...
        if collection_count == 0:
            ...
            try:
                # Create a sample document with embedding for testing
                sample_doc = {
                    "_key": "test_doc_semantic_1",
                    ...
                    EMBEDDING_FIELD: query_embedding  # Add the embedding
                }
    ...
    if not os.path.exists(fixture_path):
        ...
        try:
            # Execute search
            results = search_semantic(db, query_embedding, limit, similarity_threshold, filter_tags)
    ...
    try:
        results = search_semantic(
            ...
            fixture_params.get("similarity_threshold", similarity_threshold),
