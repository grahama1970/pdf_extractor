...
async def main() -> None:
    ...
    # Run code execution validation test
    logger.info("\n========== CODE EXECUTION VALIDATION TEST ==========")
    logger.info("Testing if LLM's factorial function executes correctly")
    logger.info("====================================================\n")
    
    code_exec_test = {
        "model": "codellama-7b",
        "prompt": "Write a Python function to calculate factorial of n. Include a test case that prints factorial(5).",
        "local": True,
        "response_schema": CodeResponse,
        "validation": {
            "type": "code_execution",
            ...
            "timeout": 60,
            "max_memory_mb": 2048,
            "expected_output": "120",  # factorial(5) = 120
            "expected_exit_code": 0,
            "field": "code"
        }
    }
    
    await get_llm_response(code_exec_test)
