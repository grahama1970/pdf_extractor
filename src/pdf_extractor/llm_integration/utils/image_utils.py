"""
Description: Utilities for handling image inputs for multimodal LLM calls.

Provides functions to download, cache, compress, resize, and encode images
(local files, URLs, Base64 strings) into the format expected by models
(typically Base64 data URIs).

Core Libraries/Concepts:
------------------------
- Pillow (PIL): For image manipulation (compression, resizing).
  (https://pillow.readthedocs.io/en/stable/)
- requests: For downloading images from URLs.
- base64: For encoding/decoding image data.
- hashlib: For generating cache keys.
- mimetypes: For guessing image types.
- pathlib: For path manipulation.

Key Functions:
--------------
- process_image_input: Main entry point to handle different image input types.
- download_and_cache_image: Downloads and caches URL images.
- compress_image: Compresses/resizes an image to meet size constraints.
- decode_base64_image: Decodes and potentially compresses a Base64 image.
- convert_image_to_base64: Converts a local image file to a Base64 data URI.

Sample I/O (process_image_input with local file):
--------------------------------------------------
Input:
  image_input: "path/to/local/image.png"
  image_directory: "/tmp/img_cache"
  max_size_kb: 500
Output:
  {"type": "image_url", "url": "data:image/jpeg;base64,..."} # Base64 of compressed JPEG
"""
