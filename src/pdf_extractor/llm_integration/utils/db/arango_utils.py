...
def connect_to_arango_client(config: Dict[str, Any]): # Added type hint
...
def get_lessons(db: StandardDatabase, role: Optional[str] = None, category: Optional[str] = None, identifier: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    # Apply safe iteration pattern
    if isinstance(cursor, Cursor):
        try:
            return [doc for doc in cursor]
        except Exception as e:
            logger.error(f"Error iterating cursor in get_lessons: {e}", exc_info=True)
            return []
    else:
        logger.error(f"Unexpected return type from db.aql.execute in get_lessons: {type(cursor)}")
        return []
...
def upsert_lesson(db: StandardDatabase, lesson: Dict[str, Any]) -> None:
    ...
    if lesson_text:
        try:
            ...
            embedding_vector = get_embedding(get_text_for_embedding(lesson_text))
            lesson['embedding'] = embedding_vector # Use 'embedding' field
            ...
            lesson['embedding'] = None # Ensure field exists even if generation fails
...
def update_lesson(db: StandardDatabase, lesson: Dict[str, Any]) -> None:
    ...
    if 'lesson' in update_payload: # Only update embedding if lesson text is part of the update
        ...
        if not lesson_text: # Handle case where lesson text is explicitly set to empty/null
             update_payload['embedding'] = None # Use 'embedding' field
    ...
    if should_update_embedding and lesson_text:
        try:
            # Use the synchronous get_embedding function
            embedding_vector = get_embedding(get_text_for_embedding(lesson_text))
            update_payload['embedding'] = embedding_vector # Use 'embedding' field
    ...
    try:
        ...
        # Pass the potentially modified update_payload, use merge=True
        collection.update(update_payload, merge=True, keep_none=False)
...
def query_lessons_by_keyword(db: StandardDatabase, keywords: List[str], limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        cursor = db.aql.execute(query, bind_vars=bind_vars) # Removed : Cursor hint
        # Apply safe iteration pattern
        if isinstance(cursor, Cursor):
             try:
                 return [doc for doc in cursor]
             except Exception as e:
                 logger.error(f"Error iterating cursor in query_lessons_by_keyword: {e}", exc_info=True)
                 return []
        else:
             logger.error(f"Unexpected return type from db.aql.execute in query_lessons_by_keyword: {type(cursor)}")
             return []
    except Exception as e:
...
def query_lessons_by_similarity(db: StandardDatabase, query_text: str, top_n: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        FILTER doc.embedding != null // Ensure embedding exists before calculating similarity - Use 'embedding' field
        LET similarity_score = COSINE_SIMILARITY(doc.embedding, @query_vector) // Use 'embedding' field
        ...
        if isinstance(cursor, Cursor):
             try:
                 ...
                 logger.error(f"Error iterating cursor in query_lessons_by_similarity: {e}", exc_info=True)
                 return [] # Return empty on iteration error
        else:
             logger.error(f"Unexpected return type from db.aql.execute in query_lessons_by_similarity: {type(cursor)}")
             return [] # Return empty on wrong type
