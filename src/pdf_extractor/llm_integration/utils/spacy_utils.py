# -*- coding: utf-8 -*-
"""
Description: Utilities for interacting with the spaCy NLP library.

Provides functions to load spaCy models (downloading if necessary), count tokens,
and truncate text based on token count.

Core Libraries/Concepts:
------------------------
- spaCy: NLP library for tokenization, etc.
  (https://spacy.io/api/language)
- subprocess: Used to run pip/curl commands for model installation.
- functools.lru_cache: Caches the loaded spaCy model for performance.

Key Functions:
--------------
- get_spacy_model: Loads a spaCy model, downloading and installing if needed.
- count_tokens: Counts tokens in a text using the loaded spaCy model.
- truncate_text_by_tokens: Truncates text to a max token count.

Sample I/O (count_tokens):
--------------------------
Input:
...
"""
import spacy
from spacy.language import Language # Import Language for type hint
